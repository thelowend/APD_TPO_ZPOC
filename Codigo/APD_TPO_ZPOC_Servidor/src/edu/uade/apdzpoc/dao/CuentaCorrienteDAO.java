...
import edu.uade.apdzpoc.excepciones.CuentaCorrienteException;
...
public class CuentaCorrienteDAO {
	...
	public CuentaCorriente findByCodigo(Integer idCtaCorriente) throws CuentaCorrienteException {
		CuentaCorriente resultado = null;
		SessionFactory sf = HibernateUtil.getSessionFactory();
		Session s = sf.openSession();
		s.beginTransaction();
		CuentaCorrienteEntity aux = (CuentaCorrienteEntity) s
				.createQuery("from CuentaCorrienteEntity where idCtaCorriente = ?").setInteger(0, idCtaCorriente)
				.uniqueResult();
		...
		if (aux != null) {
			...
			throw new CuentaCorrienteException("No se encontró la cuenta corriente " + idCtaCorriente);
		}
		return resultado;
	}

	public CuentaCorriente findByCliente(Integer idCliente) throws ClienteException {
		CuentaCorriente resultado = null;
		SessionFactory sf = HibernateUtil.getSessionFactory();
		Session s = sf.openSession();
		s.beginTransaction();
