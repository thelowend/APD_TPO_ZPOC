...
public class MovimientoAjusteDAO {
	...
	public MovimientoAjuste findrecuperadoByNro(Integer idMovimiento){
		MovimientoAjuste resultado = null;
		SessionFactory sf = HibernateUtil.getSessionFactory();
		Session s = sf.openSession();
		s.beginTransaction();
		MovimientoAjusteEntity aux = (MovimientoAjusteEntity) s.createQuery("from MovimientoAjusteEntity where idMovimiento = ?").setInteger(0, idMovimiento).uniqueResult();
	...
	public List<MovimientoAjuste> getAll(){
		...
		List<MovimientoAjusteEntity> aux = (List<MovimientoAjusteEntity>) s.createQuery("from MovimientoAjusteEntity").list();
	...
	public List<MovimientoAjuste> findBySubtipo(String estado){
		...
		s.beginTransaction();
		@SuppressWarnings("unchecked")
		List<MovimientoAjusteEntity> aux = (List<MovimientoAjusteEntity>)  s.createQuery("from MovimientoAjusteEntity where subtipo = ?").setString(0, estado).list();
		for(MovimientoAjusteEntity mae : aux)
		{
			resultado.add(this.toNegocio(mae));
		}
