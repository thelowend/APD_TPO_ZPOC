...
public class MovimientoPedidoDAO {
	...
	public MovimientoPedido findByNro(Integer idMovimiento){
		MovimientoPedido resultado = null;
		SessionFactory sf = HibernateUtil.getSessionFactory();
		Session s = sf.openSession();
		s.beginTransaction();
		MovimientoPedidoEntity aux = (MovimientoPedidoEntity) s.createQuery("from MovimientoPedidoEntity where idMovimiento = ?").setInteger(0, idMovimiento).uniqueResult();
	...
	public List<MovimientoPedido> findByPW(Integer idPedido){
		...
		List<MovimientoPedidoEntity> aux = (List<MovimientoPedidoEntity>) s.createQuery("from MovimientoPedidoEntity mpe join mpe.pw pw where pw.idPedido = ?").setInteger(0, idPedido).list();
	...
	public List<MovimientoPedido> getAll(){
		...
		s.beginTransaction();
		@SuppressWarnings("unchecked")
		List<MovimientoPedidoEntity> aux = (List<MovimientoPedidoEntity>) s.createQuery("from MovimientoPedidoEntity").list();
		for(MovimientoPedidoEntity mpe : aux)
		{
			resultado.add(this.toNegocio(mpe));
		}
