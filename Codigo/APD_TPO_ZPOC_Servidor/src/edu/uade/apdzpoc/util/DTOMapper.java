...
public class DTOMapper {
	...
	public CuentaCorrienteDTO ccDTO (CuentaCorriente cuenta){
		CuentaCorrienteDTO ccDTO = new CuentaCorrienteDTO(cuenta.getIdCtaCorriente(), cuenta.getSaldo(), cuenta.getLimMax());
		return ccDTO;
	...
	public OrdenCompraDTO ordenCompraToDTO (OrdenCompra oc){
		OrdenCompraDTO ocDTO = new OrdenCompraDTO(oc.getIdOC(), this.proveedorToDTO(oc.getProveedor()), oc.getCantidad(), oc.getEstado().toString(), this.articuloToDTO(oc.getArticulo()), this.loteToDTO(oc.getLote()), this.pedidoWebToDTO(oc.getPedidoW()), oc.getFecha());
		return ocDTO;
	}
	...
	public RemitoAlmacenDTO remitoAlmacenToDTO (RemitoAlmacen remitoAlmacen){
		...
		RemitoAlmacenDTO raDTO = new RemitoAlmacenDTO (remitoAlmacen.getIdRemito(), remitoAlmacen.getEstado(),items, remitoAlmacen.getTipo(), remitoAlmacen.getNro());
		return raDTO;
	}
	...
	public ItemRemitoAlmacenDTO itemRemitoAlmacenToDTO(ItemRemitoAlmacen ira){
		ItemRemitoAlmacenDTO iraDTO = new ItemRemitoAlmacenDTO(this.articuloToDTO(ira.getArticulo()), ira.getCantidad(),(UbicacionDTO) this.ubicacionToDTO(ira.getUbicacion()));
		return iraDTO;
	...
	public PedidoWebDTO pedidoWebToDTO(PedidoWeb pw) {
		...
		PedidoWebDTO pedidoDTO = new PedidoWebDTO(pw.getIdPedido(), clienteDTO, pw.getFechaGeneracion(), pw.getFechaDespacho(), pw.getFechaDeEntrega(), pw.getEstadoPedido(), pw.getDireccionPedido(), items);
		return pedidoDTO;
	}
	...
	/*	
		List<ItemPedidoDTO> itemsPWDTO = pwDTO.getItems();
		List<ItemPedido> items = new ArrayList<ItemPedido>();
		
		for(ItemPedidoDTO i: itemsPWDTO){
			items.add(this.dtoItemPedidoToNegocio(i));
		}
				
		Cliente cliente = this.dtoClienteToNegocio(pwDTO.getCliente());
	
		PedidoWeb pw = new PedidoWeb(pwDTO.getIdPedido(), cliente, pwDTO.getFechaGeneracion(), pwDTO.getFechaDespacho(), pwDTO.getFechaDeEntrega(), pwDTO.getEstadoPedido(), pwDTO.getDireccionPedido(), items);*/
	...
	public PagoCliente dtoPagoClienteToNegocio (PagoClienteDTO pago) throws FacturaException
	{
		...
		PagoCliente pagoNegocio = new PagoCliente (pago.getIdPago(),  pago.getMedioDePago(),pago.getFecha(),pago.getMonto(),f);
				return pagoNegocio;
	}
